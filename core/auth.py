...
def ensure_default_personal_user() -> None:
    with get_connection() as conn:
        user = conn.execute(
            "SELECT id FROM users WHERE email = ?",
            (DEFAULT_PERSONAL_USER["email"],),
        ).fetchone()
        if user:
            conn.execute(
                """
                UPDATE users
                SET name = ?, role = 'user', password_hash = ?
                WHERE id = ?
                """,
                (
                    DEFAULT_PERSONAL_USER["name"],
                    hash_secret(DEFAULT_PERSONAL_USER["password"]),
                    user["id"],
                ),
            )
            return
